...
async def setup_and_run():
    logger = LoggerManager.get_logger("simulation_consumer")
    ...
    main_queue, retry_queue, dead_letter_queue = await rabbitmq_manager.setup_queue_with_retry(
        queue_name=os.getenv("NEW_SIMULATIONS_QUEUE"),
    ...
    consumer = SimulationConsumer(
        mongo_manager.db, main_queue, logger,
        retry_queue=retry_queue,
        dead_letter_queue=dead_letter_queue,
        max_retries=config.RABBITMQ_MAX_RETRIES,
        retry_delay=config.RABBITMQ_INITIAL_RETRY_DELAY,
        monitor=monitor
    )
