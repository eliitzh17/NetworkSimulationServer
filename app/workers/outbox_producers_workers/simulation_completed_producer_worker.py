...
from app.messageBroker.producers.simulation_completed_producer import SimulationCompletedProducer
...
def main():
    async def setup_and_run():
        ...
        logger = LoggerManager.get_logger("simulation_completed_producer_worker")  
        logger.info("Starting simulation completed producer worker")
        ...
        await rabbitmq_manager.setup_exchange(config.SIMULATION_EXCHANGE, ExchangeType.TOPIC, True)
        
        # run the worker
        await run_outbox_producer_worker(
            logger,
            SimulationCompletedProducer,
            producer_args={"rabbitmq_manager": rabbitmq_manager, 
                            "exchange_name": config.SIMULATION_EXCHANGE, 
                            "db": mongo_manager.db}
        )
